...
from air.air import integrated_prediction_and_visualization_a2
from ocean.ocean import integrated_prediction_and_visualization_o2

# Set the title of the app
st.title('Best Shipping Company Prediction')
...
if st.button('Submit'):
    if transport_type == 'Air':
        result = integrated_prediction_and_visualization_a2(loading_port, loading_country, destination_port, destination_country, legs)
        st.write(f"Predicted top companies for {transport_type} leaving {loading_port} - {loading_country}\
                  to {destination_port} - {destination_country} with {legs} legs are:")
        st.dataframe(result)

    elif transport_type == 'Ocean':
        result = integrated_prediction_and_visualization_o2(loading_port, loading_country, destination_port, destination_country, legs)
        st.write(f"Predicted top companies for {transport_type} leaving {loading_port} - {loading_country}\
                  to {destination_port} - {destination_country} with {legs} legs are:")
        st.dataframe(result)
